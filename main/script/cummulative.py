...
def cummulative(df):
    ...
    def confirmed_plot(src):
        ...
        hover = HoverTool(
            tooltips=[("As at", "@Date{%F}"), ("Confirmed Cases", "@Cumulative_Case")],
            formatters={"@Date": "datetime"},
            mode="vline",
        )

        fig.line("Date", "Cumulative_Case", source=src, color="gray")
        fig.diamond(
            "Date",
            "Cumulative_Case",
            source=src,
    ...
    def recovered_plot(src):
        ...
        hover = HoverTool(
            tooltips=[("As at", "@Date{%F}"), ("Recovered Cases", "@Cumulative_Recovered")],
            formatters={"@Date": "datetime"},
            mode="vline",
        )

        fig.line("Date", "Cumulative_Recovered", source=src, color="gray")
        fig.diamond(
            "Date",
    ...
    def death_plot(src):
        ...
        hover = HoverTool(
            tooltips=[("As at", "@Date{%F}"), ("Death Cases", "@Cumulative_Death")],
            formatters={"@Date": "datetime"},
            mode="vline",
        )

        fig.line("Date", "Cumulative_Death", source=src, color="gray")
        fig.diamond(
            "Date",
            "Cumulative_Death",
            source=src,
    ...
    menu = Select(options=option, value=option[0], title="Province")
